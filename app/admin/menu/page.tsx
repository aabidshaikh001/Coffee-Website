...
export default function AdminMenuItems() {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        const res = await fetch("https://backendcoffee.onrender.com/api/menu") // replace with your real API
  ...
  const handleDeleteItem = async (id: string) => {
    if (confirm("Are you sure you want to delete this item?")) {
      try {
        const res = await fetch(`https://backendcoffee.onrender.com/api/menu/${id}`, {
          method: "DELETE",
        })
        if (res.ok) {
